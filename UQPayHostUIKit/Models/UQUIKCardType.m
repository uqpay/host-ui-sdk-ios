...
+ (instancetype)cardTypeForNumber:(NSString *)number {
    ...
    for (UQUIKCardType *cardType in [[self class] allCards]) {
...
+ (NSArray *)possibleCardTypesForNumber:(NSString *)number {
    number = [UQUIKUtil stripNonDigits:number];
    if (number.length == 0) {
...
- (NSString *)description {
    return [NSString stringWithFormat:@"UQUIKCardType %@", self.brand];
}
...
+ (NSUInteger)maxNumberLength {
    ...
    dispatch_once(&p, ^{
        for (UQUIKCardType *t in [self allCards]) {
...
+ (NSArray *)allCards
{
    ...
    dispatch_once(&p, ^{
        ...
        UQUIKCardType *visa = [[UQUIKCardType alloc] initWithBrand:UQUIKLocalizedString(CARD_TYPE_VISA)
                                                  securityCodeName:UQUIKLocalizedString(CVV_FIELD_PLACEHOLDER)
                                                          prefixes:@[@"^4\\d*"]];
        UQUIKCardType *mastercard = [[UQUIKCardType alloc] initWithBrand:UQUIKLocalizedString(CARD_TYPE_MASTER_CARD)
                                                        securityCodeName:UQUIKLocalizedString(CVC_FIELD_PLACEHOLDER)
                                                                prefixes:@[@"^(5[1-5]|222[1-9]|22[3-9]|2[3-6]|27[0-1]|2720)\\d*"]];
        UQUIKCardType *discover = [[UQUIKCardType alloc] initWithBrand:UQUIKLocalizedString(CARD_TYPE_DISCOVER)
                                                      securityCodeName:UQUIKLocalizedString(CID_FIELD_PLACEHOLDER)
                                                              prefixes:@[@"^(6011|65|64[4-9]|622)\\d*"]];
        UQUIKCardType *jcb = [[UQUIKCardType alloc] initWithBrand:UQUIKLocalizedString(CARD_TYPE_JCB)
                                                 securityCodeName:UQUIKLocalizedString(CVV_FIELD_PLACEHOLDER)
                                                         prefixes:@[@"^35\\d*"]];
        UQUIKCardType *amex = [[UQUIKCardType alloc] initWithBrand:UQUIKLocalizedString(CARD_TYPE_AMERICAN_EXPRESS)
                                                  securityCodeName:UQUIKLocalizedString(CID_FIELD_PLACEHOLDER)
                                                          prefixes:@[@"^3[47]\\d*"]
                                                   relaxedPrefixes:nil
                                                validNumberLengths:[NSIndexSet indexSetWithIndex:15]
                                                    validCvvLength:4
                                                      formatSpaces:@[@4, @10]];
        UQUIKCardType *dinersClub = [[UQUIKCardType alloc] initWithBrand:UQUIKLocalizedString(CARD_TYPE_DINERS_CLUB)
                                                        securityCodeName:UQUIKLocalizedString(CVV_FIELD_PLACEHOLDER)
                                                      ...
                                                      validNumberLengths:[NSIndexSet indexSetWithIndex:14]
                                                          validCvvLength:3
                                                            formatSpaces:nil];
        UQUIKCardType *maestro = [[UQUIKCardType alloc] initWithBrand:UQUIKLocalizedString(CARD_TYPE_MAESTRO)
                                                     securityCodeName:UQUIKLocalizedString(CVC_FIELD_PLACEHOLDER)
                                                   ...
                                                   validNumberLengths:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(12, 8)]
                                                       validCvvLength:3
                                                         formatSpaces:nil];
        UQUIKCardType *unionPay = [[UQUIKCardType alloc] initWithBrand:UQUIKLocalizedString(CARD_TYPE_UNION_PAY)
                                                      securityCodeName:UQUIKLocalizedString(CVN_FIELD_PLACEHOLDER)
                                                              prefixes:@[@"^62\\d*"]
                                                       relaxedPrefixes:nil
...
+ (NSDictionary *)cardsByBrand {
    ...
    dispatch_once(&p, ^{
        ...
        for (UQUIKCardType *cardType in [self allCards]) {
...
- (NSAttributedString *)formatNumber:(NSString *)input kerning:(CGFloat)kerning{
    input = [UQUIKUtil stripNonDigits:input];
...
- (BOOL)validAndNecessarilyCompleteNumber:(NSString *)number {
    return (number.length == self.validNumberLengths.lastIndex && [UQUIKUtil luhnValid:number]);
}

- (BOOL)validNumber:(NSString *)number {
    return ([self completeNumber:number] && [UQUIKUtil luhnValid:number]);
}

...
@interface UQUIKTextField () <UITextFieldDelegate>
@property (nonatomic, copy) NSString *previousText;
@end

@implementation UQUIKTextField
...
- (void)setPlaceholder:(NSString *)placeholder {
    ...
    [mutablePlaceholder addAttributes:@{NSForegroundColorAttributeName: [UQUIKAppearance sharedInstance].placeholderTextColor, NSFontAttributeName:[UIFont fontWithName:[UQUIKAppearance sharedInstance].fontFamily size:[UIFont labelFontSize]]} range:NSMakeRange(0, [mutablePlaceholder length])];
...
- (void)iOS9_changed {
    ...
    if ([self.editDelegate respondsToSelector:@selector(textField:willInsertText:)]) {
        // Sets _backspace = NO; in the UQUIKFormField or UQUIKFormField subclass

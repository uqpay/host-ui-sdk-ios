...
#define UQ_EXPIRY_FULL_PADDING 10
#define UQ_EXPIRY_SECTION_HEADER_HEIGHT 12

@interface UQUIKExpiryInputView ()
...
@implementation UQUIKExpiryInputView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [self.monthCollectionView registerClass:[UQUIKExpiryInputCollectionViewCell class] forCellWithReuseIdentifier:@"UQMonthCell"];
        [self.monthCollectionView registerClass:[UQUIKCollectionReusableView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"HeaderView"];
        ...
        [self.yearCollectionView registerClass:[UQUIKExpiryInputCollectionViewCell class] forCellWithReuseIdentifier:@"UQYearCell"];
        [self.yearCollectionView registerClass:[UQUIKCollectionReusableView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"HeaderView"];
        ...
        self.verticalLine.backgroundColor = [UQUIKAppearance sharedInstance].lineColor;
        ...
        NSDictionary *metrics = @{@"UQ_EXPIRY_FULL_PADDING":@UQ_EXPIRY_FULL_PADDING, @"UQ_EXPIRY_FULL_PADDING_HALF": @(UQ_EXPIRY_FULL_PADDING/2.0)};
        
        
        [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(UQ_EXPIRY_FULL_PADDING_HALF)-[monthCollectionView][verticalLine(0.5)][yearCollectionView]-(UQ_EXPIRY_FULL_PADDING_HALF)-|"
                                                                     options:0
                                                                     metrics:metrics
                                                                       views:viewBindings]];
        
        [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(UQ_EXPIRY_FULL_PADDING)-[monthCollectionView]"
                                                                     options:0
                                                                     metrics:metrics
                                                                       views:viewBindings]];
        
        [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(UQ_EXPIRY_FULL_PADDING)-[yearCollectionView]"
                                                                     options:0
                                                                     metrics:metrics
                                                                       views:viewBindings]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (collectionView == self.monthCollectionView) {
        UQUIKExpiryInputCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"UQMonthCell" forIndexPath:indexPath];
        
        cell.userInteractionEnabled = true;
        
        NSString *date = self.months[indexPath.row];
        cell.label.text = date;
        cell.backgroundColor = [UQUIKAppearance sharedInstance].formFieldBackgroundColor;
        
        cell.label.textColor = [UQUIKAppearance sharedInstance].primaryTextColor;
        
        if (self.selectedYear && self.selectedYear == self.currentYear) {
            if ([cell getInteger] < self.currentMonth) {
                cell.userInteractionEnabled = false;
                cell.label.textColor = [UQUIKAppearance sharedInstance].disabledColor;
            }
        }
        return cell;
    }
    UQUIKExpiryInputCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"UQYearCell" forIndexPath:indexPath];
...
{
    UQUIKCollectionReusableView *view = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"HeaderView" forIndexPath:indexPath];
    view.label.text = collectionView == self.yearCollectionView ? UQUIKLocalizedString(YEAR_LABEL) : UQUIKLocalizedString(MONTH_LABEL);
    return view;
}
...
- (CGSize)collectionView:(__unused UICollectionView *)collectionView layout:(__unused UICollectionViewLayout *)collectionViewLayout referenceSizeForHeaderInSection:(__unused NSInteger)section {
    return CGSizeMake(100, UQ_EXPIRY_SECTION_HEADER_HEIGHT);
}
...
{
    UQUIKExpiryInputCollectionViewCell *cell = (UQUIKExpiryInputCollectionViewCell*)[collectionView cellForItemAtIndexPath:indexPath];
...
- (void) updateVisibleCells {
    for (UQUIKExpiryInputCollectionViewCell *cell in [self.yearCollectionView visibleCells]) {
        cell.userInteractionEnabled = true;
        cell.label.textColor = [UQUIKAppearance sharedInstance].primaryTextColor;
        if (self.selectedMonth && self.selectedMonth < self.currentMonth && [cell getInteger] == self.currentYear) {
            cell.userInteractionEnabled = false;
    ...
    for (UQUIKExpiryInputCollectionViewCell *cell in [self.monthCollectionView visibleCells]) {
        cell.userInteractionEnabled = true;
        cell.label.textColor = [UQUIKAppearance sharedInstance].primaryTextColor;
        if (self.selectedYear && self.selectedYear == self.currentYear) {
            if ([cell getInteger] < self.currentMonth) {
...
- (CGSize)collectionView:(__unused UICollectionView *)collectionView layout:(__unused UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(__unused NSIndexPath *)indexPath {
    ...
    CGSize monthCellSize = CGSizeMake((self.monthCollectionView.frame.size.width - ((UQ_EXPIRY_FULL_PADDING * monthCols) + UQ_EXPIRY_FULL_PADDING + (UQ_EXPIRY_FULL_PADDING))) / monthCols , (self.monthCollectionView.frame.size.height - UQ_EXPIRY_SECTION_HEADER_HEIGHT - ((UQ_EXPIRY_FULL_PADDING * monthRows) + UQ_EXPIRY_FULL_PADDING)) / monthRows);
    ...
    return CGSizeMake((self.yearCollectionView.frame.size.width - ((UQ_EXPIRY_FULL_PADDING * yearCols) + UQ_EXPIRY_FULL_PADDING + (UQ_EXPIRY_FULL_PADDING))) / yearCols , monthCellSize.height);
}
...
- (UIEdgeInsets)collectionView:(__unused UICollectionView *)collectionView layout:(__unused UICollectionViewLayout*)collectionViewLayout insetForSectionAtIndex:(__unused NSInteger)section {
    return UIEdgeInsetsMake(UQ_EXPIRY_FULL_PADDING, UQ_EXPIRY_FULL_PADDING, UQ_EXPIRY_FULL_PADDING, UQ_EXPIRY_FULL_PADDING);
}
